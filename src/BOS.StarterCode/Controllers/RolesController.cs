            ...
            try
            {
                if (data != null) //Ensuring a non-null data
                {
                    ...
                    if (role != null) //Checking for a non-null Role object
                    {
                        ...
                        role.Name = HttpUtility.HtmlEncode(role.Name);
                        role.Description = HttpUtility.HtmlEncode(role.Description);
            ...
            try
            {
                if (data != null) //Checking for non-null input data
                {
                    ...
                    if (role != null && role.Id != Guid.Empty) //Confirming non-null role object
                    {
                        role.Name = HttpUtility.HtmlEncode(role.Name);
                        role.Description = HttpUtility.HtmlEncode(role.Description);
                        var response = await _bosAuthClient.UpdateRoleAsync<Role>(role.Id, role); //Making the API call to BOS to update a Role
                        if (response != null && response.IsSuccessStatusCode)
                        {
